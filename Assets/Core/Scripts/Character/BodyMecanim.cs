...
public class BodyMecanim : MonoBehaviour
{
	...
	public Transform weaponComplete;
	public Transform handShakeFriend;

	private bool isHandShaking = false;
	private bool pickingWeapon = false;
	...
	public void BodyAnimation(string gestureName, bool isActive)
	{
        ...
        case "DUCK":
            ...
            break;
		case "RIGHT PUNCH":
			this.animator.SetTrigger ("B_RPunch");
			break;
		case "FIGHT HITTED":
			this.animator.SetTrigger ("B_FaceHit");
			break;
		case "USE WEAPON":
			this.animator.SetTrigger ("B_UseWeapon");
			break;
		case "HEAVY HIT":
			this.animator.SetTrigger ("B_HeavyHit");
			break;
		case "USER INPUT":
		...
		case "HAND SHAKE":
			...
			break;
		case "YAWN":
			this.animator.SetTrigger ("B_Yawn");
			break;
		case "KEEP BACK":
			this.animator.SetTrigger ("B_KeepBack");
			break;
		case "IDLE":
			this.ResetAnimation();
			break;
		case "DONT KNOW WHAT TO DO":
			this.animator.SetTrigger ("B_Hardship");
			break;
		case "WAVE":
			this.animator.SetTrigger ("B_Wave");
			break;
		case "DEAD":
			this.animator.SetTrigger ("B_Dead");
			break;
		}
	}
	...
	void pickUpWeapon() {
		pickingWeapon = false;
		weaponController.linkToAent (transform, animator);
	}

	void OnAnimatorIK() {
		if (pickingWeapon && weaponController != null) {
			animator.SetLookAtWeight (1);
			animator.SetLookAtPosition (weaponComplete.position);
			animator.SetIKPositionWeight (AvatarIKGoal.RightHand, 1);
			animator.SetIKRotationWeight (AvatarIKGoal.RightHand, 1);  
			animator.SetIKPosition (AvatarIKGoal.RightHand, weaponComplete.position);
			animator.SetIKRotation (AvatarIKGoal.RightHand, weaponComplete.rotation);
		} else if (isHandShaking) {
			Vector3 handPosition = handShakeFriend.position - transform.position;
			Vector3 lookPosition = handShakeFriend.position;
			lookPosition.y = 2f;
			handPosition.Normalize ();
			handPosition = transform.position + 2f * handPosition;
			handPosition.y = 1.3f;
			animator.SetLookAtWeight (1);
			animator.SetLookAtPosition (lookPosition);
			animator.SetIKPositionWeight (AvatarIKGoal.RightHand, 1);
			animator.SetIKPosition (AvatarIKGoal.RightHand, handPosition);
		} else if (!pickingWeapon && !isHandShaking) {
    ...
    public void ResetAnimation()
    {
		...
		this.animator.SetBool ("B_Idle_Fight", false);
		this.animator.SetFloat ("WalkSpeed", 0);
		this.animator.ResetTrigger ("B_Yawn");
